...
class RelatorioContaReceber extends TPage
{
    ...
    function __construct()
    {
        ...
        error_reporting(0);
        // creates the form
        $this->form = new BootstrapFormBuilder('form_RelatorioContaReceber_report');
        $this->form->setFormTitle( 'Relatório Contas Receber' );
    ...
    function onGenerate()
    {
        try
        {
            ...
            // define the query
            $query = 'SELECT conta.descricao, conta_receber.valor, conta_receber.data_vencimento, conta_receber.valor_recebido, pessoa.nome, conta_receber.status                            
                      FROM  conta_receber, pessoa, conta
                      WHERE conta_receber.pessoa_id = pessoa.id  AND
                            conta_receber.conta_id = conta.id  AND
                            conta_receber.data_vencimento BETWEEN :data_inicio AND :data_fim';
            ...
            if ($rows)
            {
                ...
                if (!empty($table))
                {
                    ...
                    $table->setHeaderCallback( function($table) {
                        ...
                        $table->addCell('Valor Recebido', 'center', 'title');
                    ...
                    {                       
                        ...
                        $table->addCell($row['valor_recebido'], 'right', $style);                        
                    ...
                    $table->addCell(number_format($ValorRecebido,2,',','.'), 'right', 'footer', 3);
